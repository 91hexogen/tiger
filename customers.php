...
<div class="main col-md-3 p-3 p-md-1-5 m-md-3 text-info">
  <div class="col-md-1-3 p-lg-1 mx-auto my-1">
    <h2 class="display-5 font-weight-normal text-warning">CUSTOMERS</h2>
    <p class="lead font-weight-small text-light">Konsole Labs customers who have sent most push notifications.</p>
...
var chart_labels_24 = [<?php while($row = mysqli_fetch_array($result1)){echo "'".$row["appName"]."',";} ?>];
var push_dataset_24 = [<?php while($row = mysqli_fetch_array($result2)) {echo "".$row["number"].",";} ?>];

var ctx = document.getElementById("forecast").getContext('2d');
var config = {
   type: 'horizontalBar',
   data: {
       labels: chart_labels_24,
       datasets: [{
           label: "Pushes",
           data: push_dataset_24,
           backgroundColor: [
             'rgba(255,99,132,0.2)',
             'rgba(255,159,64,0.2)',
             'rgba(255,205,86,0.2)',
...
$("#0").click(function() {
 ...
 data.datasets[0].data = push_dataset_24;
 data.labels = chart_labels_24;
