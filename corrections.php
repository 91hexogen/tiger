...
// 24 HOURS
$query1 = "SELECT appName FROM Notification WHERE postTime / 1000 >= UNIX_TIMESTAMP(NOW() - INTERVAL 24 HOUR) GROUP BY appName HAVING COUNT(nMessage) > 1 ORDER BY COUNT(nMessage) DESC LIMIT 10";
$query2 = "SELECT COUNT(nMessage) AS number FROM Notification WHERE postTime / 1000 >= UNIX_TIMESTAMP(NOW() - INTERVAL 24 HOUR) GROUP BY appName HAVING COUNT(nMessage) > 1 ORDER BY COUNT(nMessage) DESC LIMIT 10";
// WEEK
$query3 = "SELECT appName FROM Notification WHERE postTime / 1000 >= UNIX_TIMESTAMP(NOW() - INTERVAL 7 DAY) GROUP BY appName HAVING COUNT(nMessage) > 1 ORDER BY COUNT(nMessage) DESC LIMIT 10";
$query4 = "SELECT COUNT(nMessage) AS number FROM Notification WHERE postTime / 1000 >= UNIX_TIMESTAMP(NOW() - INTERVAL 7 DAY) GROUP BY appName HAVING COUNT(nMessage) > 1 ORDER BY COUNT(nMessage) DESC LIMIT 10";
// MONTH
$query5 = "SELECT appName FROM Notification WHERE postTime / 1000 >= UNIX_TIMESTAMP(NOW() - INTERVAL 30 DAY) GROUP BY appName HAVING COUNT(nMessage) > 1 ORDER BY COUNT(nMessage) DESC LIMIT 10";
$query6 = "SELECT COUNT(nMessage) AS number FROM Notification WHERE postTime / 1000 >= UNIX_TIMESTAMP(NOW() - INTERVAL 30 DAY) GROUP BY appName HAVING COUNT(nMessage) > 1 ORDER BY COUNT(nMessage) DESC LIMIT 10";
// YEAR
$query7 = "SELECT appName FROM Notification WHERE postTime / 1000 >= UNIX_TIMESTAMP(NOW() - INTERVAL 1 YEAR) GROUP BY appName HAVING COUNT(nMessage) > 1 ORDER BY COUNT(nMessage) DESC LIMIT 10";
$query8 = "SELECT COUNT(nMessage) AS number FROM Notification WHERE postTime / 1000 >= UNIX_TIMESTAMP(NOW() - INTERVAL 1 YEAR) GROUP BY appName HAVING COUNT(nMessage) > 1 ORDER BY COUNT(nMessage) DESC LIMIT 10";
// ALL TIME
...
$result1 = mysqli_query($connect, $query1);$result2 = mysqli_query($connect, $query2);$result3 = mysqli_query($connect, $query3);$result4 = mysqli_query($connect, $query4);
$result5 = mysqli_query($connect, $query5);$result6 = mysqli_query($connect, $query6);$result7 = mysqli_query($connect, $query7);$result8 = mysqli_query($connect, $query8);
$result9 = mysqli_query($connect, $query9);$result10 = mysqli_query($connect, $query10);
?>
...
<div class="main col-md-3 p-3 p-md-1-5 m-md-3 text-info">
  <div class="col-md-1-3 p-lg-1 mx-auto my-1">
    <h2 class="display-5 font-weight-normal text-warning">CORRECTIONS</h2>
    <p class="lead font-weight-small text-light">Apps that have most frequently corrected their notifications.</p>
